...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 1   | **Point the AWS relay at this app.** PawaPay calls the API Gateway; it must forward to `https://DOMAIN/api/payments/pawapay/callback` with the body byte-identical and the signature headers intact — see the relay contract in `docs/guides/payments-runbook.md` | Payments succeed, money leaves people's accounts, **nobody ever gets a ticket**. No error anywhere                      |
| 1b  | Set `PAWAPAY_SIGNATURE_AUTHORITY` and `PAWAPAY_SIGNATURE_PATH` to the gateway's address **before** enabling `PAWAPAY_ENFORCE_SIGNATURE`                                                                                                                           | Signature verification fails on every genuine callback                                                                  |
