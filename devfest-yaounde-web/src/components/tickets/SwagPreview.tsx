...
/**
 * The swag that comes with a paid tier — PHASE14 edit.
 *
 ...
 *
 * Images are placeholders. The name is the real information, which is why it
 * is in the popover and in the accessible name rather than only in a file.
 */
export function SwagPreview({ items }: { items: LocalizedString[] }) {
      ...
      <ul className="swag-row mt-3 flex flex-wrap items-end gap-3">
        {items.map((item, i) => (
          <li
            ...
            style={{ ["--swag-tilt" as string]: `${i % 2 === 0 ? -3 : 3}deg` }}
          >
            ...
            <span className="sr-only">{item[locale]}</span>
          </li>
        ))}
      </ul>
    </div>
